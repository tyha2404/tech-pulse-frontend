...
export const AdminDashboard: React.FC<AdminDashboardProps> = () => {
  ...
  const handleResetBreakers = async () => {
    try {
      ...
      setMessage('✅ Đã reset toàn bộ Circuit Breaker về trạng thái hoạt động bình thường (CLOSED)!');
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-[#756e60] font-sans">
        <RefreshCw className="w-6 h-6 animate-spin text-[#4b5563] mb-2.5" />
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-12 animate-fadeIn font-serif">
      ...
      <div className="flex flex-col justify-between gap-4 rounded-2xl border border-[#e7e2d9] bg-gradient-to-r from-[#fbf9f5] via-[#f7f3ea] to-[#f4eee2] p-6 shadow-2xs md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 font-sans text-xs text-[#635c50]">
            Theo dõi trạng thái từng nguồn tin, độ trễ pipeline AI, trạng thái Circuit Breakers và nhật ký cào tin thời gian thực.
          </p>
        </div>
      ...
      {message && (
        <div className="p-3.5 rounded-xl border border-indigo-200 bg-indigo-50/80 text-indigo-900 text-xs font-sans font-medium flex items-center gap-2 shadow-2xs">
          <Radio className="w-4 h-4 text-indigo-700 animate-pulse shrink-0" />
          <span>{message}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 font-sans">
        <div className="rounded-xl border border-[#e7e2d9] bg-white p-4 shadow-xs">
          <div className="flex items-center justify-between text-[#756e60] mb-1.5">
          ...
          <div className="text-[11px] text-[#8c8475] mt-1">
            <strong className="text-emerald-800">{metrics?.analyzed_articles}</strong> bài đã phân tích AI
          </div>
        </div>

        <div className="rounded-xl border border-[#e7e2d9] bg-white p-4 shadow-xs">
          <div className="flex items-center justify-between text-[#756e60] mb-1.5">
            <span className="text-[11px] font-bold uppercase tracking-wider">Bài Tinh Tuyển (≥8.0)</span>
        ...
        <div className="rounded-xl border border-[#e7e2d9] bg-white p-4 shadow-xs">
          <div className="flex items-center justify-between text-[#756e60] mb-1.5">
            <span className="text-[11px] font-bold uppercase tracking-wider">Nguồn Hoạt Động</span>
            <Globe className="w-4 h-4 text-emerald-700" />
          </div>
          <div className="font-serif text-2xl font-bold text-[#1c1f24]">
            {metrics?.active_sources} <span className="font-sans text-xs font-normal text-[#8c8475]">/ {metrics?.total_sources}</span>
          </div>
          <div className="text-[11px] text-emerald-800 mt-1 flex items-center gap-1 font-semibold">
            <CheckCircle2 className="w-3 h-3 text-emerald-700" /> {metrics?.healthy_sources} nguồn khỏe mạnh
          </div>
        </div>

        <div className="rounded-xl border border-[#e7e2d9] bg-white p-4 shadow-xs">
          <div className="flex items-center justify-between text-[#756e60] mb-1.5">
            <span className="text-[11px] font-bold uppercase tracking-wider">Nguồn Gặp Sự Cố</span>
            <AlertTriangle className="w-4 h-4 text-amber-600" />
          </div>
          <div className={`font-serif text-2xl font-bold ${(metrics?.error_sources || 0) > 0 ? 'text-amber-800' : 'text-[#1c1f24]'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="rounded-xl border border-[#e7e2d9] bg-white p-5 shadow-xs">
          <h3 className="font-serif text-sm font-bold text-[#1c1f24] flex items-center gap-2 mb-3">
          ...
          <div className="space-y-2.5 font-sans">
            {metrics?.circuit_breakers_status && Object.keys(metrics.circuit_breakers_status).length > 0 ? (
              Object.entries(metrics.circuit_breakers_status).map(([model, status]) => (
                <div key={model} className="flex items-center justify-between p-2.5 rounded-lg border border-[#ede7dc] bg-[#faf8f4]">
                  <div className="flex items-center gap-2">
                    <div className={`w-2.5 h-2.5 rounded-full ${status === 'CLOSED' ? 'bg-emerald-600' : (status === 'HALF_OPEN' ? 'bg-amber-500' : 'bg-rose-600 animate-pulse')}`} />
                    <span className="text-xs font-semibold text-[#1c1f24] font-mono">{model}</span>
                  </div>
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${
                    status === 'CLOSED' ? 'bg-emerald-100 text-emerald-800' :
                    (status === 'HALF_OPEN' ? 'bg-amber-100 text-amber-800' : 'bg-rose-100 text-rose-800')
                  }`}>
                    {status}
                  </span>
                </div>
              ...
              <div className="p-3 rounded-lg border border-[#ede7dc] bg-[#faf8f4] text-xs text-[#756e60] flex items-center justify-between">
                <span>Tất cả Circuit Breakers đều hoạt động ổn định.</span>
                <span className="text-emerald-800 font-bold uppercase text-[10px] bg-emerald-100 px-2 py-0.5 rounded">CLOSED</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-[#e7e2d9] bg-white p-5 shadow-xs">
          ...
          <div className="space-y-3 font-sans">
            {metrics?.ai_model_distribution && Object.keys(metrics.ai_model_distribution).length > 0 ? (
                ...
                return (
                  <div key={model} className="space-y-1">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="text-[#756e60]">{count} bài ({percentage}%)</span>
                    </div>
                    <div className="h-1.5 w-full bg-[#eee9df] rounded-full overflow-hidden">
                      <div 
                        className="h-full bg-[#2c313a] rounded-full" 
              ...
              <div className="text-xs text-[#8c8475] py-4">Chưa có dữ liệu phân bổ mô hình AI.</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-[#e7e2d9] bg-white shadow-xs overflow-hidden">
        <div className="border-b border-[#e7e2d9] bg-[#f7f4ed] px-5 py-3.5 flex items-center justify-between">
          <h3 className="font-serif text-sm font-bold text-[#1c1f24] flex items-center gap-2">
            <Globe className="w-4 h-4 text-emerald-700" />
          ...
          <table className="w-full text-left text-xs font-sans border-collapse min-w-[640px]">
            ...
            <tbody className="divide-y divide-[#ede8df]">
              {metrics?.source_health.map((src) => (
                <tr key={src.id} className="hover:bg-[#fbf9f5] transition">
                  <td className="p-3.5 font-semibold text-[#1c1f24]">
                    <a href={src.url} target="_blank" rel="noreferrer" className="hover:text-indigo-900 hover:underline inline-flex items-center gap-1">
                      {src.name} <ArrowUpRight className="w-3 h-3 text-[#8c8475]" />
                    </a>
                  </td>
                  <td className="p-3.5">
                    <span className="px-2 py-0.5 rounded bg-[#f4efe6] text-[#635d52] font-mono text-[10px] font-bold uppercase">
                      {src.source_type}
                    </span>
                  </td>
                  <td className="p-3.5">
                    {src.status === 'healthy' ? (
                      ...
                      <span className="inline-flex items-center gap-1 text-rose-800 font-semibold text-[11px]">
                        <XCircle className="w-3.5 h-3.5 text-rose-700" /> Error
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-amber-800 font-semibold text-[11px]">
                        <Clock className="w-3.5 h-3.5 text-amber-700" /> {src.status}
                      </span>
                    )}
                  </td>
                  <td className="p-3.5 font-mono text-[#1c1f24] font-medium">
                    {src.articles_count}
                  </td>
                  ...
                  <td className="p-3.5 text-rose-800 max-w-[200px] truncate font-mono text-[10px]" title={src.last_error || ''}>
                    {src.last_error || '—'}
                  </td>
                  <td className="p-3.5 text-right">
                    <button
                      ...
                      className="cursor-pointer px-2.5 py-1 rounded border border-[#ded7ca] bg-[#f4efe6] text-[#2c313a] hover:bg-[#eae4d7] text-[11px] font-semibold transition inline-flex items-center gap-1"
                    >
                      <RefreshCw className={`w-3 h-3 ${crawlingSourceId === src.id ? 'animate-spin' : ''}`} />
                      Cào ngay
                    </button>
                  </td>
                </tr>
            ...
            <tbody className="divide-y divide-[#ede8df]">
              {metrics?.recent_runs && metrics.recent_runs.length > 0 ? (
                metrics.recent_runs.map((run) => (
                  <tr key={run.id} className="hover:bg-[#fbf9f5] transition">
                    ...
                    <td className="p-3.5 font-mono text-emerald-800 font-bold">+{run.articles_new}</td>
                    <td className="p-3.5">
                ...
                <tr>
                  <td colSpan={6} className="py-6 text-center text-[#8c8475] text-xs">
